...
# Find all current lifetime members that purchased memberships before the five year window
ind_info_lt = indy_info %>% 
  filter(MEMBERSHIP_TYPE_CODE == "L") %>% 
  select(ID_DEMO) %>% 
  filter(!(ID_DEMO %in% membership$ID_DEMO))

# create years for each and join them
...
records_new = rbind(records_minus5,records_minus4,records_minus3,records_minus2,records_lastYear) %>% 
...
# Annual or lifetime or non-member (get multi-year memberships and lifetime purchases in later code chunks)
...
# Joint memberships (not used in this analysis)
...
# a: If ann_num_years is NA (meaning no digit was found in MEMBERHSIP_STATUS) and the 
#    word "Annual" was found in MEMEBRSHIP_STATUS then we classify the purchase as a
...
membership_join <- membership_join %>% 
  mutate(ann_num_years = ifelse(is.na(ann_num_years) & grepl("Annual",MEMBERSHIP_LEVEL), 1,
...
# Remove Last Year to not include it in the counts
...
# emails_events <- read_csv('D:/Group Folder/Data/pred_emails_events.csv')
...
# eng_cleaned <- read_csv("D:/Group Folder/Data/Cleaned data sets/engagement_cleaned.csv")
eng_cleaned <- read_csv("2 Data Cleaned Files/engagement_cleaned.csv")
...
# setwd("D:/Group Folder/Data/Cleaned data sets")
# write.csv(training_df, "Data_member_retention_training_final.csv", row.names = FALSE)
# write.csv(future_prediction_df, "Data_member_retention_future_prediction_final.csv", row.names = FALSE)

write.csv(training_df, "3 Data Generated Files/Data_member_retention_training_final.csv", row.names = FALSE)
write.csv(future_prediction_df, "3 Data Generated Files/Data_member_retention_future_prediction_final.csv", row.names = FALSE)
