...
Original file is located at
    https://colab.research.google.com/drive/115fCqrRTcv1EuxChaEAJExVH8Z_HL0mj
"""
...
def Normalization(X_train, X_test, X_valid = None):
    # Normalization
    ...
    columns_to_scale = ['AGE', 'UMN_event',
    ...
    if X_valid is not None:
      scaled_features_valid = X_valid[columns_to_scale]
      scaled_data_valid = ss.transform(scaled_features_valid)
      X_valid[columns_to_scale] = scaled_data_valid
      return X_train, X_test, X_valid
    return X_train, X_test


def feature_selection(X_train, X_test, X_valid = None):
    pca = PCA()
...
def model_building(model_name, X, y):
    ...
    if model_name == 'rf':
        ...
        best_params = {'colsample_bytree': 0.8, 'max_depth': 25, 'n_estimators': 700, 'num_leaves': 200}
        rf_cv = GridSearchCV(rf, best_params, scoring='roc_auc', cv=5)
        rf_cv.fit(X, y)
    ...
    if model_name == 'LGBM':
        ...
        # }
        #param_grid = {'max_depth': [10,25]}
        lgbm_cv = LGBMClassifier(boosting_type='gbdt', objective='binary', random_state=42, colsample_bytree= 0.8, n_estimators= 700, num_leaves= 200, max_depth=25)
        #lgbm_cv = GridSearchCV(lgbm_cv, param_grid, n_jobs=-1, cv=5, scoring='roc_auc')
        lgbm_cv.fit(X, y)
...
if __name__ == "__main__":
     ...
     if normalization:
         X_train, X_test, X_valid = Normalization(X_train, X_test, X_valid)
     if feature_selection_ind:\
         X_train, X_test, X_valid = feature_selection(X_train, X_test, X_valid)
     if imbalance_ind:
...
pred_lgbm_prba = est.predict_proba(X_test)
pred_lgbm_no = est.predict(X_test)
# print(pred_lgbm_prba)
print(roc_auc_score(y_test, pred_lgbm_prba[:,1]))

est.best_score_

print(classification_report(y_test, pred_lgbm_no))

# Get all measurement
columns_name = ['Accuracy', 'Precision', 'Recall', 'F1-Score']
accuracy = accuracy_score(y_test, pred_lgbm_no)
print('Accuracy of LGBM Model is {0}'.format(round(accuracy,4)))
print(np.unique(y_test, return_counts=True))
print(confusion_matrix(y_test, pred_lgbm_no))

a= est.predict_proba(X_valid)[:,1]
