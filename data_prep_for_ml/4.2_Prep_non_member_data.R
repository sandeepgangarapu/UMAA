...
# Group1 : Lifemembers
# There are two ways to get Life members. 1. From Indi file 2. From membership file
...
# In this case we consider the max value of the two
current_life_members <- current_life_members[, max_yr := max(FISCAL_YEAR), .(ID_DEMO)][FISCAL_YEAR == max_yr, .(ID_DEMO, FISCAL_YEAR)]


life_members_per_yr <- copy(current_life_members)[, `:=`(FY0 = current_fy-1,
                                                         FY1=current_fy-2,
                                                         FY2=current_fy-3,
                                                         FY3=current_fy-4,
                                                         FY4=current_fy-5)]

life_members_per_yr <- melt.data.table(life_members_per_yr,
                                       measure.vars = c("FY0", "FY1", "FY2", "FY3", "FY4"),
                                       variable.name = "yr", value.name = "FY")

life_members_per_yr <- life_members_per_yr[, member_status := ifelse(FISCAL_YEAR<= FY, 'life_member', 'non_member')]

life_members_per_yr <- life_members_per_yr[, .(ID_DEMO, FY, member_status)][, FISCAL_YEAR:= FY][, FY:=NULL]
life_members_per_yr <- unique(life_members_per_yr)
...
# some of them dont have any digits, in that case, replace NA with 1 as most of these memberships are for 1 year. if not all.
...
non_members <- copy(indy_info_main)[!(ID_DEMO %in% annual_members_per_yr$ID_DEMO)][!(ID_DEMO %in% life_members_per_yr$ID_DEMO),.(ID_DEMO)]
...
main_df_train <- copy(main_df)

main_df_train <- dcast.data.table(main_df_train, ID_DEMO~member_status, fun=length) 

#if everything is accurate, the tot of every record should be 5
...
emails_events_member <- merge.data.table(main_df_train, emails_events, by = c('ID_DEMO'), all.x=TRUE)
...
last_5_avg_eng <- eng_cleaned[(ID_DEMO %in% main_df_train$ID_DEMO) & (YEAR_FISCAL >= previous_fy - 5)]
